...
public class CadastroAparelhosController implements Initializable {
    ...
    private void abrirJanelaCategoria() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/categorias.fxml"));
    ...
    void excluirAparelho(ActionEvent event) {
    	Aparelho excluir_aparel = TableViewAparelhos.getSelectionModel().getSelectedItem();
    	
    	if(excluir_aparel != null) {
    		aparelhos.remove(excluir_aparel);
    	}
    }
